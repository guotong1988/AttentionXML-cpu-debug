...
def build_tree_by_level(sparse_data_x, sparse_data_y, mlb, eps: float, max_leaf: int, levels: list, groups_path):
    ...
    labels_feature = normalize(csr_matrix(sparse_y.T) @ csc_matrix(sparse_x))
    ...
    for i in range(len(levels)-1, -1, -1):
        if os.path.exists(F'{groups_path}-Level-{i}.npy'):
            ...
            q = [(labels_i, labels_feature[labels_i]) for labels_i in labels_list]
            break
    if q is None:
        q = [(np.arange(labels_feature.shape[0]), labels_feature)]
    while q:
        labels_list = np.asarray([x[0] for x in q])
        assert sum(len(labels) for labels in labels_list) == labels_feature.shape[0]
        ...
        next_q = []
        for node_index, node_feature in q:
            if len(node_index) > max_leaf:
                next_q += list(split_node(node_index, node_feature, eps))
        q = next_q
