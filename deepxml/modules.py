...
class AttentionWeights(nn.Module):
    ...
    def __init__(self, labels_num, hidden_size, device_ids=None):
        ...
        if device.type == "cpu":
            self.emb = nn.ModuleList(nn.Embedding(size, hidden_size, sparse=True).to(device)
    ...
    # inputs: [batch,label_num]
    def forward(self, inputs: torch.Tensor):
        outputs = torch.zeros(*inputs.size(), self.hidden_size, device=inputs.device) # [batch, label_num, hidden_dim]
        for left, right, emb in zip(self.group_offset[:-1], self.group_offset[1:], self.emb):
            index = (left <= inputs) & (inputs < right) # bool类型 [batch, label_num]
            group_inputs = torch._cast_Long((inputs[index] - left).to(emb.weight.device))
...
class FastMLAttention(nn.Module):
    ...
    def __init__(self, labels_num, hidden_size, parallel_attn=False):
        ...
        if not parallel_attn:
            ...
            nn.init.xavier_uniform_(self.attention.weight)
    # candidates: [batch, label_num+1]
    def forward(self, inputs, masks, candidates, attn_weights: nn.Module):
        masks = torch.unsqueeze(masks, 1)   # [batch, 1, seq_len]
        attn_inputs = inputs.transpose(1, 2)    # [batch, hidden_dim, seq_len]
        attn_weights2 = self.attention(candidates) if hasattr(self, 'attention') else attn_weights(candidates) # [batch, label_num+1, hidden_dim]
        attention = (attn_weights2 @ attn_inputs).masked_fill(~masks, -np.inf)  # [batch, label_num+1, seq_len]
        attention = F.softmax(attention, -1)    # [batch, label_num+1, seq_len]
        return attention @ inputs   # [batch, label_num+1, hidden_dim]
