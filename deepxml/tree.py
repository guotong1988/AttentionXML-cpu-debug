...
class FastAttentionXML(object):
    ...
    @staticmethod # 由叶子的label得到group的label
    ...
    def train_level(self, level, train_x, train_y, valid_x, valid_y):
        ...
        if level == 0:
            ...
            return train_y, model.predict(train_loader, k=self.top), model.predict(valid_loader, k=self.top) # 使用上一层预测的group作为下层的group
        else:
            train_group_y, train_group, valid_group = self.train_level(level - 1, train_x, train_y, valid_x, valid_y) # 上一层预测的结果
            torch.cuda.empty_cache()

            logger.info('Getting Candidates') # beam search
            ...
            for i, labels in tqdm(enumerate(group_labels), leave=False, desc='Parents'):
                y_start, y_end = train_group_y.indptr[i], train_group_y.indptr[i + 1]
                positive = set(train_group_y.indices[y_start: y_end])
            ...
            if not os.path.exists(model.model_path):
                ...
                model.network.emb.load_state_dict(last_model.emb.state_dict())
                model.network.lstm.load_state_dict(last_model.lstm.state_dict())
                model.network.linear.load_state_dict(last_model.linear.state_dict())
